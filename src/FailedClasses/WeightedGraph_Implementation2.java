...
import restaurantLookup.Edge;
import restaurantLookup.GraphNode;
import restaurantLookup.Restaurant;

public class WeightedGraph_Implementation2<T> {
	...
	public WeightedGraph_Implementation2(GraphNode<Restaurant> src) throws Exception{
		try{
		String mypath = "src/restaurantLookup/relfiles/RESTAURANT_LIST.xls";
		UtilityMethods ul = new UtilityMethods(); 
		Restaurant[] restarr = ul.excelDataToRestaurant(ul.ReadExcel(mypath));
		...
		/*Restaurant[] restcopy = restarr.clone();
		int attachmentpoint = -1;
		
		Collections.sort(Arrays.asList(restcopy), new Comparator<Restaurant>() {
		    @Override
		    public int compare(Restaurant c1, Restaurant c2) {
		        return 	Double.compare(c1.compareGeoLocation(src.getData()), c2.compareGeoLocation(src.getData()));
		    }
		});
		*/
		...
		for(int i = 0; i < restarr.length; i++ ){
			GraphNode<Restaurant> g = new GraphNode(restarr[i]);
			gn.add(g);
			/*if(restarr[i].compareTo(restcopy[0]) == 0){
				attachmentpoint  = i;
			} */
		...
		/*if(attachmentpoint > -1){
			gn.add(src);
			gn.get(59).addEdge(gn.get(59).getData().compareGeoLocation(gn.get(attachmentpoint).getData()), gn.get(attachmentpoint),true);
		} */
		//System.out.println(gn.get(1).getData().toString());
		gn.add(src);
	...
	public String BreadthFirst(GraphNode x){
		String returnval = "";
		boolean visited[] = new boolean[copyGraph.size()];
		LinkedList<GraphNode> queue = new LinkedList<GraphNode>();
		visited[copyGraph.indexOf(x)] = true; 
		queue.addAll(gn);
		while(queue.size() != 0 ){
			x = queue.poll();
			returnval = returnval + "Hit Node: " + x.getData().toString();
			Iterator<Edge> i = queue.
		}
